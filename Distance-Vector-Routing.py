...
def broadcast_costs():
    ...
    for neighbor_addr, neighbor in get_neighbors().items():
        ...
        #    sock.sendto(str[idx:idx+max_bytes], key2addr(neighbor_addr))#
        sock.sendto(str, key2addr(neighbor_addr))

def setup_server(host, port):
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    #sock = socket.socket('192.168.188.128', 20000)
    try:
        sock.bind((host, port))
